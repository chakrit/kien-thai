...
def _has_prose(output: Path) -> bool:
    """Failed and timed-out generations leave a 0-byte output.md behind."""
    return output.is_file() and bool(output.read_text(encoding="utf-8").strip())
...
def claude_arm(eval_name: str, iteration: Path) -> ArmRef | None:
    ...
    """
    same_iteration = iteration / eval_name / "claude" / "with_skill" / "output.md"
    if _has_prose(same_iteration):
        return ArmRef(same_iteration, co_generated=True)

    elsewhere = [
        (int(output.parents[3].name.split("-")[1]), output)
        for output in WORKSPACE.glob(f"iteration-*/{eval_name}/claude/with_skill/output.md")
        if _has_prose(output)
    ]
    if not elsewhere:
        return None

    newest = max(elsewhere, key=lambda numbered: numbered[0])[1]
    return ArmRef(newest, co_generated=False)
...
def _signal_row(label: str, signals: Signals) -> str:
    forbidden = ", ".join(signals.forbidden_hits) if signals.forbidden_hits else "—"
    return (
        f"| {label:<16} | {signals.chars:>5} | {signals.paragraphs:>3} | "
        f"{signals.connective_density:>4} | {signals.exclamations:>3} | {forbidden} |"
    )
...
def _source_lines(draft: Path, arm: ArmRef | None) -> list[str]:
    ...
    return [
        draft_line,
        f"- **Claude + skill:** `{arm.path.relative_to(WORKSPACE.parent)}`",
        _CO_GENERATED_NOTE if arm.co_generated else _CONTAMINATED_NOTE,
    ]


def render_comparison(draft: Path) -> str:
    """One eval's comparison page: provenance, signal table, both prose blocks.

    Signals are advisory — forbidden-phrase hits, connective density, exclamations,
    length. They route the ear; they are never the verdict.
    """
    eval_name = draft.parents[2].name
    draft_text = draft.read_text(encoding="utf-8")
    arm = claude_arm(eval_name, draft.parents[3])
    claude_text = arm.path.read_text(encoding="utf-8") if arm else ""

    rows = [_signal_row("typhoon-draft", mechanical_signals(draft_text))]
    if arm:
        rows.append(_signal_row("claude+skill", mechanical_signals(claude_text)))
    ...
    sections = [
        ...
        *rows,
        ...
        "",
        claude_text.strip() if arm else "_(no Claude output yet)_",
        "",
    ]
    return "\n".join(sections) + "\n"
